...
double normalPDF(const VectorXd& mean, const MatrixXd& covar,
                 const VectorXd& input)
{
  // -0.5 * (x-mu)^T * Sigma^-1 * (x-mu)
  double inside_exp =
      -0.5 * (mean - input).dot(covar.inverse() * (mean - input));
  // (1/sqrt( (2*PI)^k * |Sigma| )) * exp(-0.5 * (x-mu)^T * Sigma^-1 * (x-mu))
  return pow(pow(2 * M_PI, input.size() * covar.determinant()), -0.5) *
         exp(inside_exp);
}

double normalPDFRealTime(const VectorXd& mean, const MatrixXd& covar,
                         const VectorXd& input)
{
  // These things must be preallocated or precomputed, or they will dynamically
  // allocate memory.
  ...
  double normalization =
      pow(pow(2 * M_PI, input.size() * covar_determinant), -0.5);
...
int main(int n_args, char** args)
{
  ...
  cout << "  normalPDF(mean,covar,input)=" << normalPDF(mean, covar, input)
       << endl;
  cout << "  normalPDFRealTime(mean,covar,input)="
       << normalPDFRealTime(mean, covar, input) << endl;
}
