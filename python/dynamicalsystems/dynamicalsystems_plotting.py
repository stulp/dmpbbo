...
def checkIfVectorContainsTime(ts):
    ...
    if not monotonically_increasing or not constant_dt:
        print(
            "WARNING: First column does not seem to represent time. Please change the way plotDynamicalSystem is called, or use plotDynamicalSystemDeprecated instead."
        )
...
def plotDynamicalSystemComparison(data1, data2, name1, name2, axs, axs_diff):
    ...
    plt.setp(
        lines_diff, linestyle="-", linewidth=2, color=(0.50, 0.00, 0.00), label="diff"
    )
    plt.legend()
