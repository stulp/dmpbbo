...
FunctionApproximator* from_jsonpickle(const nlohmann::json& json) {
  
  string class_name = json.at("py/object").get<string>();
  
  if (class_name.find("FunctionApproximatorRBFN") != string::npos) {
    return FunctionApproximatorRBFN::from_jsonpickle(json);
    
  } else if (class_name.find("FunctionApproximatorLWR") != string::npos) {
    return FunctionApproximatorLWR::from_jsonpickle(json);
