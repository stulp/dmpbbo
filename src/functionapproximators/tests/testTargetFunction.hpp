...
#ifndef TESTTARGETFUNCTION_HPP
#define TESTTARGETFUNCTION_HPP
 ...
 */
inline void targetFunction(Eigen::VectorXi n_samples_per_dim,
                           Eigen::MatrixXd& inputs, Eigen::MatrixXd& targets)
{
  ...
  if (n_dims == 1) {
    ...
    targets = 3 * (-inputs.col(0)).array().exp() *
              (2 * inputs.col(0).array().pow(2)).sin();

  } else {
    // 2D Function, similar to the example and graph here:
    ...
    Eigen::VectorXd x1 =
        Eigen::VectorXd::LinSpaced(n_samples_per_dim[0], -2.0, 2.0);
    Eigen::VectorXd x2 =
        Eigen::VectorXd::LinSpaced(n_samples_per_dim[1], -2.0, 2.0);
    ...
    targets = 2.5 * inputs.col(0).array() *
              exp(-inputs.col(0).array().pow(2) - inputs.col(1).array().pow(2));
  }
}

#endif  //  #ifndef TESTTARGETFUNCTION_HPP
