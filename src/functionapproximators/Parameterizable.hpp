...
// If EIGEN_RUNTIME_NO_MALLOC is defined, we want to check for dynamic memory allocation.
// Thus we define ENTERING_REAL_TIME_CRITICAL_CODE and EXITING_REAL_TIME_CRITICAL_CODE to start/stop
...
#else 
// EIGEN_RUNTIME_NO_MALLOC is not defined, not need to do any checks on real-time code. Simply set
