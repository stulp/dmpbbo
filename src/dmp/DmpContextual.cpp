...
void DmpContextual::set_policy_parameter_function_goal(
...
void DmpContextual::set_policy_parameter_function_duration(
    FunctionApproximator* function_approximator)
{
  // Output of this function approximator should always be 1D (duration is time
  // is 1D)
...
// In the end, all of the below train(...) variants call the pure virtual
// function virtual void  train(const std::vector<Trajectory>& trajectories,
// const std::vector<Eigen::MatrixXd>& task_parameters, std::string
// save_directory, bool overwrite) = 0;
    ...
    std::string save_directory, bool overwrite)
{
  if (policy_parameter_function_goal_.size() > 0 ||
    ...
    if (policy_parameter_function_goal_.size() > 0) {
      ...
      for (int i_dim = 0; i_dim < dim_orig(); i_dim++) {
        ...
        if (!save_directory.empty())
          save_directory_cur =
              save_directory + "/dim" + to_string(i_dim) + "_goal";

        policy_parameter_function_goal_[i_dim]->train(
            inputs, targets.col(i_dim), save_directory_cur, overwrite);
      }
    }
    ...
    if (policy_parameter_function_duration_ != NULL) {
      ...
      policy_parameter_function_duration_->train(inputs, targets,
                                                 save_directory_cur, overwrite);
    }
  }
...
void DmpContextual::set_task_parameters(const MatrixXd& task_parameters)
{
  ...
  task_parameters_ =
      task_parameters;  // This will be used for the forcing term later
  ...
  if (policy_parameter_function_duration_ != NULL) {
    ...
    policy_parameter_function_duration_->predict(task_parameters,
                                                 duration_as_matrix);
...
void DmpContextual::checkTrainTrajectories(
  ...
  for (unsigned int i_demo = 1; i_demo < n_demonstrations; i_demo++) {
    ...
    if (fabs(first_duration - trajectories[i_demo].duration()) > 10e-4) {
      if (policy_parameter_function_duration_ == NULL) {
        ...
        cerr << "WARNING: Duration of demonstrations differ (" << first_duration
             << "!=" << trajectories[i_demo].duration() << ")" << endl;
        cerr << "         See "
                "DmpContextual::set_policy_parameter_function_duration(...) on "
                "how to fix this. "
             << endl;
      }
    }
    ...
    double sum_abs_diff =
        (first_y_init.array() - trajectories[i_demo].initial_y().array())
            .abs()
            .sum();
    if (sum_abs_diff > 10e-7) {
      cerr << __FILE__ << ":" << __LINE__ << ":";
      cerr << "WARNING: Initial states of demonstrations differ ( ["
           << first_y_init.transpose() << "] != [ "
           << trajectories[i_demo].initial_y().transpose() << "] )" << endl;
    }

    // Difference between final states
    sum_abs_diff =
        (first_y_attr.array() - trajectories[i_demo].final_y().array())
            .abs()
            .sum();
    if (sum_abs_diff > 10e-7 && policy_parameter_function_goal_.size() == 0) {
      cerr << __FILE__ << ":" << __LINE__ << ":";
      cerr << "WARNING: Final states of demonstrations differ ( ["
           << first_y_attr.transpose() << "] != [ "
           << trajectories[i_demo].final_y().transpose() << "] )" << endl;
      cerr << "         See "
              "DmpContextual::set_policy_parameter_function_goal(...) on how "
              "to fix this. "
           << endl;
    }
  }
