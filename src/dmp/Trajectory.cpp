...
Trajectory::Trajectory(const Eigen::VectorXd& ts, const Eigen::MatrixXd& ys,
    ...
    : ts_(ts), ys_(ys), yds_(yds), ydds_(ydds), misc_(misc)
{
  ...
  if (misc_.cols() == 0) misc_ = MatrixXd(n_time_steps, 0);
...
void Trajectory::set_misc(const Eigen::MatrixXd& misc)
{
  if (misc.rows() == ts_.size()) {
    ...
    cerr << "misc must have 1 or " << ts_.size() << " rows, but it has "
         << misc.rows() << endl;
  }
}
...
void Trajectory::append(const Trajectory& trajectory)
{
  ...
  if (ydds_.row(length() - 1).isZero() || trajectory.ydds().row(0).isZero())
    assert(ydds_.row(length() - 1).isZero() &&
           trajectory.ydds().row(0).isZero());
  else
  ...
  new_ydds << ydds_,
      trajectory.ydds().block(1, 0, trajectory.length() - 1, dim());
  ydds_ = new_ydds;
  ...
  if (dim_misc() == 0) {
    ...
    new_misc << misc_,
        trajectory.misc().block(1, 0, trajectory.length() - 1, dim_misc());
    misc_ = new_misc;
  }
}
...
Trajectory Trajectory::generateMinJerkTrajectory(const VectorXd& ts,
                                                 const VectorXd& y_from,
                                                 const VectorXd& y_to)
{
  int n_time_steps = ts.size();
  int n_dims = y_from.size();

  MatrixXd ys(n_time_steps, n_dims), yds(n_time_steps, n_dims),
  ...
  for (int i_dim = 0; i_dim < n_dims; i_dim++) {
    ...
    ys.col(i_dim) =
        y_from[i_dim] +
        A[i_dim] * (6 * tss.pow(5) - 15 * tss.pow(4) + 10 * tss.pow(3));

    yds.col(i_dim) =
        (A[i_dim] / D) * (30 * tss.pow(4) - 60 * tss.pow(3) + 30 * tss.pow(2));

    ydds.col(i_dim) =
        (A[i_dim] / (D * D)) * (120 * tss.pow(3) - 180 * tss.pow(2) + 60 * tss);
  }

  return Trajectory(ts, ys, yds, ydds);
}

Trajectory Trajectory::generatePolynomialTrajectory(
    const VectorXd& ts, const VectorXd& y_from, const VectorXd& yd_from,
    const VectorXd& ydd_from, const VectorXd& y_to, const VectorXd& yd_to,
    const VectorXd& ydd_to)
{
  ...
  VectorXd a3 = -10 * y_from - 6 * yd_from - 2.5 * ydd_from + 10 * y_to -
                4 * yd_to + 0.5 * ydd_to;
  VectorXd a4 =
      15 * y_from + 8 * yd_from + 2 * ydd_from - 15 * y_to + 7 * yd_to - ydd_to;
  VectorXd a5 = -6 * y_from - 3 * yd_from - 0.5 * ydd_from + 6 * y_to -
                3 * yd_to + 0.5 * ydd_to;
  ...
  for (int i = 0; i < ts.size(); i++) {
    ...
    ys.row(i) = a0 + a1 * t + a2 * pow(t, 2) + a3 * pow(t, 3) + a4 * pow(t, 4) +
                a5 * pow(t, 5);
    yds.row(i) = a1 + 2 * a2 * t + 3 * a3 * pow(t, 2) + 4 * a4 * pow(t, 3) +
                 5 * a5 * pow(t, 4);
    ydds.row(i) =
        2 * a2 + 6 * a3 * t + 12 * a4 * pow(t, 2) + 20 * a5 * pow(t, 3);
  }
...
Trajectory Trajectory::generatePolynomialTrajectoryThroughViapoint(
    const VectorXd& ts, const VectorXd& y_from,
    const VectorXd& y_yd_ydd_viapoint, double viapoint_time,
    const VectorXd& y_to)
{
  int n_dims = y_from.size();
  assert(n_dims == y_to.size());
  assert(3 * n_dims ==
         y_yd_ydd_viapoint.size());  // Contains y, yd and ydd, so *3

  int n_time_steps = ts.size();

  int viapoint_time_step = 0;
  while (viapoint_time_step < n_time_steps &&
         ts[viapoint_time_step] < viapoint_time)
    viapoint_time_step++;

  if (viapoint_time_step >= n_time_steps) {
    cerr << __FILE__ << ":" << __LINE__ << ":";
    cerr << "ERROR: the time vector does not contain any time smaller than "
         << viapoint_time << ". Returning min-jerk trajectory WITHOUT viapoint."
         << endl;
    return Trajectory();
  }
  ...
  Trajectory traj = Trajectory::generatePolynomialTrajectory(
      ts.segment(0, viapoint_time_step + 1), y_from, yd_from, ydd_from,
      y_viapoint, yd_viapoint, ydd_viapoint);
  traj.append(Trajectory::generatePolynomialTrajectory(
      ts.segment(viapoint_time_step, n_time_steps - viapoint_time_step),
      y_viapoint, yd_viapoint, ydd_viapoint, y_to, yd_to, ydd_to));
...
{
  MatrixXd traj_matrix(trajectory.length(),
                       1 + 3 * trajectory.dim() + trajectory.dim_misc());
  traj_matrix << trajectory.ts_, trajectory.ys_, trajectory.yds_,
      trajectory.ydds_, trajectory.misc_;
  output << traj_matrix << endl;
  return output;
}

bool Trajectory::saveToFile(std::string directory, std::string filename,
...
bool Trajectory::saveToFile(std::string filename, bool overwrite) const
{
  MatrixXd traj_matrix;
  asMatrix(traj_matrix);
...
Trajectory Trajectory::readFromFile(std::string filename, int n_dims_misc)
{
  MatrixXd traj_matrix;
