...
DmpWithGainSchedules::DmpWithGainSchedules(
    Dmp* dmp, std::vector<FunctionApproximator*> function_approximators_gains)
    : Dmp(*dmp)
{
...
DmpWithGainSchedules::DmpWithGainSchedules(
  ...
  : Dmp( tau, y_init,
y_attr,function_approximators,alpha_spring_damper,goal_system,phase_system,gating_system,scaling)
{
  initFunctionApproximatorsExtDims(function_approximators_gains);
}
...
std::vector<FunctionApproximator*> function_approximators, double
alpha_spring_damper, DynamicalSystem* goal_system, DynamicalSystem*
phase_system, DynamicalSystem* gating_system, std::vector<FunctionApproximator*>
function_approximators_gains, ForcingTermScaling scaling) :
Dmp(n_dims_dmp,function_approximators,alpha_spring_damper,goal_system,phase_system,gating_system,scaling)
{
  initFunctionApproximatorsExtDims(function_approximators_gains);
}

DmpWithGainSchedules::DmpWithGainSchedules(double tau, Eigen::VectorXd y_init,
Eigen::VectorXd y_attr, std::vector<FunctionApproximator*>
function_approximators, DmpType dmp_type, std::vector<FunctionApproximator*>
function_approximators_gains, ForcingTermScaling scaling) : Dmp(tau, y_init,
y_attr,function_approximators,dmp_type,scaling), forcing_term_scaling_(scaling)
{
  initFunctionApproximatorsExtDims(function_approximators_gains);
}
...
void DmpWithGainSchedules::initFunctionApproximatorsExtDims(
    vector<FunctionApproximator*> function_approximators_gains)
{
  if (function_approximators_gains.empty()) return;

  // Doesn't necessarily have to be the same
  // assert(dim_orig()==(int)function_approximators_exdim.size());

  function_approximators_gains_ =
      vector<FunctionApproximator*>(function_approximators_gains.size());
  for (unsigned int dd = 0; dd < function_approximators_gains.size(); dd++) {
    if (function_approximators_gains[dd] == NULL)
      ...
      function_approximators_gains_[dd] =
          function_approximators_gains[dd]->clone();
  }
}
...
DmpWithGainSchedules* DmpWithGainSchedules::clone(void) const
{
  vector<FunctionApproximator*> function_approximators_gains;
  for (unsigned int ff = 0; ff < function_approximators_gains_.size(); ff++)
    function_approximators_gains.push_back(
        function_approximators_gains_[ff]->clone());

  return new DmpWithGainSchedules(Dmp::clone(), function_approximators_gains);
}

void DmpWithGainSchedules::computeFunctionApproximatorOutputExtendedDimensions(
  ...
  for (int i_dim = 0; i_dim < dim_orig(); i_dim++) {
    if (function_approximators_gains_[i_dim] != NULL) {
      if (function_approximators_gains_[i_dim]->isTrained()) {
        if (T == 1) {
          function_approximators_gains_[i_dim]->predict(
              phase_state, fa_gains_outputs_one_prealloc_);
          fa_output.col(i_dim) = fa_gains_outputs_one_prealloc_;
        } else {
          function_approximators_gains_[i_dim]->predict(
              phase_state, fa_gains_outputs_prealloc_);
...
void DmpWithGainSchedules::integrateStart(
    Eigen::Ref<Eigen::VectorXd> x, Eigen::Ref<Eigen::VectorXd> xd,
...
void DmpWithGainSchedules::integrateStep(
...
void DmpWithGainSchedules::analyticalSolution(const Eigen::VectorXd& ts,
                                              Eigen::MatrixXd& xs,
                                              Eigen::MatrixXd& xds,
                                              ...
                                              Eigen::MatrixXd& fa_gains) const
{
  ...
  computeFunctionApproximatorOutputExtendedDimensions(xs.PHASEM(xs.rows()),
                                                      fa_gains);
}
...
void DmpWithGainSchedules::train(const Trajectory& trajectory,
                                 std::string save_directory, bool overwrite)
{
  // First, train the DMP
  ...
  // The dimensionality of the extra variables in the trajectory must be the
  // same as the number of function approximators.
  ...
  for (unsigned int dd = 0; dd < function_approximators_gains_.size(); dd++) {
    // This is just boring stuff to figure out if and where to store the results
    // of training
    ...
    if (function_approximators_gains_[dd] == NULL) {
      ...
      cerr << "WARNING: function approximator cannot be trained because it is "
              "NULL."
           << endl;
    } else {
      if (function_approximators_gains_[dd]->isTrained())
        function_approximators_gains_[dd]->reTrain(
            xs_phase, cur_target, save_directory_dim, overwrite);
      else
        function_approximators_gains_[dd]->train(xs_phase, cur_target,
                                                 save_directory_dim, overwrite);
    }
  }
}

/*
void DmpWithGainSchedules::getSelectableParameters(set<string>&
selectable_values_labels) const {
...
void DmpWithGainSchedules::setSelectedParameters(const set<string>&
selected_values_labels)
{
  Dmp::setSelectedParameters(selected_values_labels);
  Eigen::VectorXi lengths_per_dimension_dmp =
Dmp::getVectorLengthsPerDimension();
  ...
  for (int dd=0; dd<dim_gains(); dd++)
  {
    if (function_approximators_gains_[dd]!=NULL)
    {
      if (function_approximators_gains_[dd]->isTrained())
      {
        ...
        lengths_per_dimension_gains[dd] =
function_approximators_gains_[dd]->getParameterVectorSelectedSize();
      }
    }
  }

  VectorXi lengths_per_dimension(dim_orig()+dim_gains());
  lengths_per_dimension << lengths_per_dimension_dmp,
lengths_per_dimension_gains;
  setVectorLengthsPerDimension(lengths_per_dimension);

}

void DmpWithGainSchedules::getParameterVectorMask(const std::set<std::string>
...
//
//   // This makes sure that the indices for each function approximator are
different
...
int DmpWithGainSchedules::getParameterVectorAllSize(void) const
{
  ...
  for (unsigned int dd=0; dd<function_approximators_gains_.size(); dd++)
    total_size +=
function_approximators_gains_[dd]->getParameterVectorAllSize();
...
void DmpWithGainSchedules::setParameterVectorAll(const VectorXd& values)
{
  ...
  {
    int n_parameters_required =
function_approximators_gains_[dd]->getParameterVectorAllSize(); cur_values =
values.segment(last_index-n_parameters_required,n_parameters_required);
