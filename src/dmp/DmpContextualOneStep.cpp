...
DmpContextualOneStep::DmpContextualOneStep(
...
void DmpContextualOneStep::train(const vector<Trajectory>& trajectories,
                                 ...
                                 string save_directory, bool overwrite)
{
  ...
  for (unsigned int i_demo = 0; i_demo < n_demonstrations; i_demo++) {
    ...
    if (i_demo == 0) {
      ...
      // This is the first time task_parameters_ is set, because this is the
      // first time we know n_task_parameters. We set it so that
      // set_task_parameters can check if
      // task_parameters_.cols()==n_task_parameters
    ...
    if (cur_task_parameters.rows() == 1 && n_time_steps > 1) {
      MatrixXd cur_task_parameters_tmp =
          cur_task_parameters.replicate(n_time_steps, 1);
      cur_task_parameters = cur_task_parameters_tmp;
    }
    assert(n_time_steps == cur_task_parameters.rows());

    computeFunctionApproximatorInputsAndTargets(
        trajectories[i_demo], cur_fa_input_phase, cur_fa_target);
    ...
    all_fa_inputs.bottomRightCorner(n_time_steps, n_task_parameters) =
        cur_task_parameters;
  ...
  // We have all inputs and targets. Now train the function approximator for
  // each dimension.
  ...
  for (int dd = 0; dd < dim_orig(); dd++) {
    ...
    function_approximator(dd)->train(all_fa_inputs, fa_target,
