...
DmpContextualTwoStep::DmpContextualTwoStep(
    int n_dims_dmp, std::vector<FunctionApproximator*> function_approximators,
    FunctionApproximator* policy_parameter_function, DmpType dmp_type)
    : DmpContextual(n_dims_dmp, function_approximators, dmp_type)
{
  policy_parameter_function_ =
      vector<vector<FunctionApproximator*> >(dim_orig());
    ...
    Eigen::MatrixXd& fa_output) const
{
  ...
  if (task_parameters_.rows() == 0) {
    // When the task parameters are not set, we cannot compute the output of the
  ...
  if (task_parameters.rows() == 1) {
    ...
    task_parameters =
        task_parameters.col(0).transpose().replicate(n_time_steps, 1).eval();
  }
  ...
  for (int dd = 0; dd < dim_orig(); dd++) {
    int n_parameters =
        function_approximator(dd)->getParameterVectorSelectedSize();
  ...
  // The parameters of the function_approximators have been set, get their
  // outputs now.
  for (int dd = 0; dd < dim_orig(); dd++) {
...
bool DmpContextualTwoStep::isTrained(void) const
{
  for (int dd = 0; dd < dim_orig(); dd++)
    for (unsigned int pp = 0; pp < policy_parameter_function_[dd].size(); pp++)
      if (!policy_parameter_function_[dd][pp]->isTrained()) return false;

  return true;
}

void DmpContextualTwoStep::train(const vector<Trajectory>& trajectories,
                                 const vector<MatrixXd>& task_parameters,
                                 string save_directory, bool overwrite)
{
  // Here's the basic structure of this function
  // 1) Do some checks
  // 2) Train a separate Dmp for each demonstration, and get the resulting model
  // parameters 3) Gather all task parameter values for all demonstrations 4)
  // Train the policy parameter function for each dimension and each model
  // parameter
  ...
  // Then check if the trajectories have the same duration and initial/final
  // state Later on, if they are not the same, they should be learned also.
  ...
  for (unsigned int i_demo = 0; i_demo < n_demonstrations; i_demo++) {
    ...
    for (int i_dim = 0; i_dim < dim_orig(); i_dim++) {
      ...
      function_approximator(i_dim)->getParameterVectorSelected(
          cur_model_parameters);
      // cout << cur_model_parameters << endl;
      if (i_demo == 0)
        all_model_parameters[i_dim].resize(n_demonstrations,
                                           cur_model_parameters.size());
      else
        assert(cur_model_parameters.size() ==
               all_model_parameters[i_dim].cols());
  ...
  // This is the first time task_parameters_ is set, because this is the first
  // time we know n_task_parameters. We set it so that set_task_parameters can
  // check if task_parameters_.cols()==n_task_parameters
  ...
  for (unsigned int i_demo = 0; i_demo < n_demonstrations; i_demo++) {
    ...
    for (int i_time = 1; i_time < task_parameters[i_demo].rows(); i_time++) {
      if ((cur_task_parameters_t0.array() !=
           task_parameters[i_demo].row(i_time).array())
              .any()) {
        cerr << __FILE__ << ":" << __LINE__ << ":";
        cerr << "WARNING. For DmpContextualTwoStep, task parameters may not "
                "vary over time during training. Using task parameters at t=0 "
                "only."
             << endl;
      }
    }

    // Take the first row, i.e. at time_i = 0. We checked above if they are
    // constant over time.
    inputs.row(i_demo) = cur_task_parameters_t0;
  }

  //-----------------------------------------------------
  // 4) Train the policy parameter function for each dimension and each model
  // parameter
  ...
  for (int i_dim = 0; i_dim < dim_orig(); i_dim++) {
    ...
    for (int i_pol_par = 1; i_pol_par < n_pol_pars; i_pol_par++) {
      policy_parameter_function_[i_dim].push_back(
          policy_parameter_function_[i_dim][0]->clone());
    ...
    for (int i_pol_par = 0; i_pol_par < n_pol_pars; i_pol_par++) {
      ...
      if (!save_directory.empty())
        save_directory_cur = save_directory + "/dim" + to_string(i_dim) +
                             "_polpar" + to_string(i_pol_par);

      policy_parameter_function_[i_dim][i_pol_par]->train(
          inputs, targets, save_directory_cur, overwrite);
    }
  }
}
