...
class TaskSolverDmpWithGains(TaskSolver):
    ...
    def perform_rollout_dmp_sched(self, dmp_sched):
        ...
        ts, ys_cur, yds_cur, ydds_cur, schedules, ys_des, yds_des, ydds_des =\
          force_field_simulator.perform_rollout(dmp_sched, self._integrate_time, self._n_time_steps)
...
def main():
    ...
    schedule = np.full((n_time_steps, n_dims), 0.5)
    traj.misc = schedule
    ...
    dmp = DmpWithSchedules.from_traj_sched(traj, function_apps, function_apps_schedules,
                                           min_schedules=0.1, max_schedules=2.0)
